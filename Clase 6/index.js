...
app.put("/products/:id", (req, res) => {
    ...
    if (!product) {
        res.send("No existe el producto en la lista.")
    } else{
        product.name = name
        product.color = color
        res.send(product)
...
//PATCH -> actualiza una porción de la información
app.patch("/products/:id", (req, res) => {
    const { id } = req.params
    const prod = req.body
    const index = products.findIndex((e) => e.id == id)
    //Desestructuración de objetos -> copia o una extracción de valores a variables -> { } / [ ]
    //Spread Operator -> copia o expandir los valores un objeto o un array en otro
    if(index == -1){
        res.send("Error. No se encontró el id.")
    } else {
        const newPatch = {...products[index], ...prod}
        // const newPatch = {...prod, ...products[index]} -> no olvidar el orden de las variables es fundamental!
        products.splice(index, 1, newPatch)
        res.send(newPatch)
    }

    // const product = products.find((e) => e.id == id)
    // //Comnpara y pisa propiedades específicamente de un objeto
